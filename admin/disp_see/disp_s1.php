<?php
  ...
  $query1 = "SELECT * FROM registration WHERE approved = 0";
  $result = mysqli_query($con, $query1);
  $rr = mysqli_num_rows($result);
  if (!$rr) {
    echo "<h2 style='color:red;color:#ff0000;font-family:Acme;'>Zero Students Waiting for Approval.</h2>";
  } else {
          ...
          <tr>
              ...
              <td>
                  ...
                  <?php if ($row['approved'] == 0): ?>
                      <form method="post">
                          <input type="hidden" name="usn" value="<?php echo htmlspecialchars($row['USN']); ?>" />
                          <button type="submit" name="approve_user" onclick="return confirm('Are you sure you want to approve this user?');">
                              Approve
                          </button>
                      </form>
                  <?php else: ?>
