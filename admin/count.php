<?php
    ...
    #Count students waiting for approval
    $pendingQuery = "SELECT COUNT(*) AS pending_count FROM registration WHERE approved = 0";
    ...
    #Count students who are approved
    $approvedQuery = "SELECT COUNT(*) AS approved_count FROM registration WHERE approved = 1";
    $approvedResult = mysqli_query($con, $approvedQuery);

    if ($approvedResult) {
        $approvedRow = mysqli_fetch_assoc($approvedResult);
        $approvedCount = $approvedRow['approved_count'];
    } else {
        $approvedCount = 0;
    }
